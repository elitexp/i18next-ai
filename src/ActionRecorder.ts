export default class ActionRecorder {
  ...
  constructor(fileName: string, isReportMode: boolean) {
  ...
  public hasRootKey(key: string) {
    const rootKey = key.split("::")[0];
    return this.addedKeys.indexOf(rootKey) !== -1;
  }
  ...
  private getMessageForRemovedKeys() {
    if (this.removedKeys.length > 0) {
      const prefix = this.isReportMode
        ? "Orphaned keys found in"
        : "Removed keys from";
